...
class hr_applicant(osv.osv):
    ...
    _columns={
              ...
              'resume_note':fields.text('Resume'),   
    ...
    def message_new(self, cr, uid, msg, custom_values=None, context=None):
       ...
       # body=smart_str(body)
        ...
        for i in range(len(msg['attachments'])):
                ...
                att_name=msg['attachments'][i][0]
                a=att_name.split('.')
                #======================================Find Current working path================================== 
                cwd_path = os.getcwd()
                report_path = cwd_path + '/temp'
                if not os.path.exists(report_path):
                    os.makedirs(report_path)
                #print report_path
                text_file = report_path +'/' +"resume.txt"
                resume_file = report_path +'/' +"resume." + a[-1]
                #================================================================================================= 
                if a[-1] == 'doc':
                    filename=open(resume_file , 'w')
                    filename.write(str(msg['attachments'][i][1]))
                    filename.close()
                    commands.getoutput("catdoc '%s'> '%s'"%(resume_file,text_file))
                    resume_file=open(text_file , 'r')
                    resume = resume_file.read()
                elif a[-1] == 'docx':
                     filename=open(resume_file , 'w')
                     filename.write(str(msg['attachments'][i][1]))
                     filename.close()
                     commands.getoutput("docx2txt.pl '%s' '%s' " %(resume_file,text_file))
                     resume_file=open(text_file, 'r')
                     resume = resume_file.read()
                elif a[-1] == 'pdf':
                     filename=open(resume_file , 'w')
                     filename.write(str(msg['attachments'][i][1]))
                     filename.close()
                     commands.getoutput("pdftotext '%s' "%(resume_file))
                     resume_file=open(text_file, 'r')
                     resume = resume_file.read()
